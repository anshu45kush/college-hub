...
// @route   GET /api/users
// @desc    Get all users with filtering and roll number sorting
// @access  Private (Admin only)
router.get('/', authenticate, authorize('admin'), async (req, res) => {
  try {
    const { role, branch, semester, isActive, search } = req.query;
    ...
    // Sort students by rollNumber, others by createdAt
    const sortObj = {};
    if (role === 'student') sortObj.rollNumber = 1;
...
router.get('/students', authenticate, authorize('teacher', 'admin'), async (req, res) => {
  try {
    ...
    const students = await User.find(filter)
      .select('-password')
      .sort({ rollNumber: 1 }); // sort students by rollNumber
...
// @route   POST /api/users
// @desc    Create new user with duplicate email/rollNumber check
// @access  Private (Admin only)
router.post('/', authenticate, authorize('admin'), async (req, res) => {
  try {
    const { name, email, password, role, branch, semester, rollNumber, employeeId, department } = req.body;
    ...
    const existingUser = await User.findOne({
      $or: [
        { email },
        { rollNumber: rollNumber || null } // null if undefined for teachers
      ]
    });
    ...
    if (existingUser) {
      ...
      else if (existingUser.rollNumber === rollNumber) message = 'Roll number already exists';
...
router.get('/stats/overview', authenticate, authorize('admin'), async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { totalUsers, activeUsers, roleStats: stats }
    });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Server error while fetching user statistics', error: error.message });
  }
});
